...
Estado C018R.3-FIX1: mitigado en scripts mediante paquete completo PostgreSQL + storage + inventario documental semántico + validación DB → archivo → SHA-256 en backup, verify y restore temporal.
Cierre definitivo: ejecutar preparación de storage persistente, backup y restauración temporal en el VPS real antes del piloto.
...
Cierre definitivo: ejecutar el script en VPS y confirmar que `docker inspect dentia-backend` muestra el mount.
...
Cierre definitivo: crear archivo real con permisos `600`, rotar secretos antes del piloto y no compartir `docker compose config` sin redacción.
