...
docker restart "$DENTIA_DB_CONTAINER" "$DENTIA_BACKEND_CONTAINER" "$DENTIA_FRONTEND_CONTAINER" "$DENTIA_WEBSITE_CONTAINER" >/dev/null
sleep 5
dentia_compose ps
...
if ! dentia_wait_http "$DENTIA_PRODUCTION_WEBSITE_URL" 20 2; then
  docker logs --tail 120 "$DENTIA_WEBSITE_CONTAINER" || true
  dentia_fail "Website did not become reachable after restart."
fi
