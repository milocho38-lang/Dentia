...
import { getCompany, listSites } from "@/services/organizationService";
...
const templateOriginLabel: Record<string, string> = {
  DENTIA_LIBRARY: "Biblioteca Dentia",
  CLONED_FROM_DENTIA: "Copia de Biblioteca Dentia",
  CLINIC_CUSTOM: "Creada por la clínica",
};
...
function CheckList({ items, selected, onChange, disabled = false }: { items: { id: string; name: string }[]; selected: string[]; onChange: (next: string[]) => void; disabled?: boolean }) {
  return (
    <div className="max-h-40 space-y-1 overflow-y-auto rounded-xl border border-slate-200 p-2">
      {items.length === 0 ? <p className="p-2 text-xs text-slate-400">Sin opciones disponibles.</p> : items.map((item) => (
        <label key={item.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
          <input type="checkbox" disabled={disabled} checked={selected.includes(item.id)} onChange={(event) => onChange(event.target.checked ? [...selected, item.id] : selected.filter((id) => id !== item.id))} />
          {item.name}
        </label>
      ))}
    </div>
...
export function ConsentTemplatesPage() {
  ...
  const canReviewContent = hasPermission("consent.template.review_content");
  ...
  const [tenantCountry, setTenantCountry] = useState<"CO" | "CL" | null>(null);
  ...
  function resetCreateModalState() {
    const next = createEmptyConsentTemplateForm();
    if (tenantCountry) {
      next.country_code = tenantCountry;
      next.language_code = `es-${tenantCountry}`;
    }
    setCreateForm(next);
    setError(null);
  }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [list, kindItems, variableItems, siteItems, procedureItems, company] = await Promise.all([
        listConsentTemplates(query),
        getConsentDocumentKinds(),
        getConsentVariables(),
        listSites(),
        listProcedureCatalog(),
        isPlatformAdmin ? Promise.resolve(null) : getCompany(),
      ]);
      const libraryQuery = new URLSearchParams();
      if (search) libraryQuery.set("q", search);
      if (isPlatformAdmin && country) libraryQuery.set("country", country);
      ...
      if (company) {
        const companyCountry = company.country?.trim().toLowerCase() === "chile" ? "CL" : "CO";
        setTenantCountry(companyCountry);
        setCreateForm((current) => ({
          ...current,
          country_code: companyCountry,
          language_code: `es-${companyCountry}`,
        }));
      }
      if (selected) {
  ...
  }, [activeTab, canReadLibrary, country, filterProcedure, filterSite, filterSpecialty, isPlatformAdmin, kind, search, selected, status]);
  ...
  async function showPreview() {
    ...
    setError(null);
    try {
      const saved = selected.template_origin === "DENTIA_LIBRARY" ? activeDraft : await persistDraft();
  ...
  async function publishDraft() {
    ...
    try {
      const saved = selected.template_origin === "DENTIA_LIBRARY" ? activeDraft : await persistDraft();
      let reviewed = saved;
      if (!saved.clinic_content_review_confirmed) {
        const confirmed = window.confirm("Confirmo que la clínica revisó el contenido de esta plantilla y considera que es adecuado para su utilización con pacientes. La clínica asume responsabilidad sobre el contenido publicado.");
        if (!confirmed) return;
        reviewed = await reviewConsentVersionContent(selected.id, saved.id);
      }
      if (!window.confirm("La versión revisada quedará inmutable y reemplazará prospectivamente a la publicada actual. ¿Publicar?")) return;
      await publishConsentVersion(selected.id, reviewed.id);
  ...
  async function reviewExistingVersion(version: ConsentVersion) {
    if (!selected) return;
    const confirmed = window.confirm("Confirmo que la clínica revisó el contenido de esta plantilla y considera que es adecuado para su utilización con pacientes. La clínica asume responsabilidad sobre el contenido publicado.");
    if (!confirmed) return;
    setSaving(true);
    setError(null);
    try {
      await reviewConsentVersionContent(selected.id, version.id);
      setSuccess("Contenido revisado y adoptado por la clínica.");
      await refreshSelected(selected.id);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "No fue posible confirmar la revisión del contenido.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      <Alert tone="info">Cada clínica es responsable de revisar y aprobar el contenido de las plantillas que utiliza. Dentia asegura el procedimiento técnico, la trazabilidad y la integridad del consentimiento.</Alert>
      ...
      <section className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-4">
        ...
        {isPlatformAdmin ? <select aria-label="Filtrar por país" value={country} onChange={(event) => setCountry(event.target.value)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm"><option value="">Todos los países</option><option value="CO">Colombia</option><option value="CL">Chile</option></select> : <div className="flex min-h-11 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-bold text-slate-600">País de la empresa: {tenantCountry === "CL" ? "Chile" : "Colombia"}</div>}
      ...
      {loading ? <div className="flex min-h-48 items-center justify-center"><Spinner /></div> : activeTab === "library" ? (
        <section className="grid gap-4 lg:grid-cols-2">
          ...
          {libraryItems.map((item) => {
            ...
            const installable = item.document_type === "INFORMED_CONSENT" && (item.signer_scope === "PATIENT_SELF" || item.signer_scope === "PATIENT_OR_RESPONSIBLE_ADULT" || item.signer_scope === "RESPONSIBLE_ADULT_REQUIRED" || item.signer_scope === "ADULT_SELF" || item.signer_scope === "ADULT_OR_REPRESENTATIVE" || item.signer_scope === "REPRESENTATIVE_REQUIRED");
            return (
              <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 rounded-xl bg-amber-50 p-3 text-xs text-amber-800">{installable ? "Plantilla sugerida por Dentia. Puedes agregarla sin cambios o crear una copia editable; la clínica debe revisar su contenido antes de publicarla." : "Documento disponible como referencia. Su tipo de flujo no está habilitado como consentimiento estándar."}</div>
                <div className={`mt-4 grid gap-3 ${isPlatformAdmin ? "sm:grid-cols-2" : "grid-cols-1"}`}>
                  {[co, cl].filter(isLibraryVersion).map((version) => (
                    <div key={version.id} className="rounded-xl border border-slate-200 p-3">
                      ...
                      {version.legal_review_status === "APPROVED" && version.clinical_review_status === "APPROVED" && <p className="mt-1 text-xs font-bold text-green-700">Equivalencia de normalización registrada · {version.review_reference ?? "sin referencia"}</p>}
                      {version.legal_review_status !== "APPROVED" && <p className="mt-1 text-xs text-slate-500">La equivalencia de normalización permanece informativa; la clínica revisará el contenido tenant antes de publicarlo.</p>}
                      <p className="mt-1 break-all font-mono text-[10px] text-slate-400">{version.normalized_content_sha256}</p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {canInstallLibrary && installable && version.normalization_status !== "BLOCKED" && version.electronic_readiness_status !== "BLOCKED" && <button type="button" disabled={saving || item.installed_exact} onClick={() => void libraryAction(version.id, "install")} className="rounded-lg bg-dentia-primary px-3 py-1.5 text-xs font-black text-white disabled:opacity-50">Agregar plantilla sugerida</button>}
          ...
          <section>
            {!selected ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">Selecciona una plantilla para editar su borrador o consultar el historial.</div> : (
              <div className="space-y-5">
                ...
                {selected.draft_versions.length > 0 && (
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    {activeDraft && <div className="mt-5 space-y-5">
                      <div className="grid gap-4 lg:grid-cols-2"><label className="text-sm font-bold text-slate-700">Título<input value={draft.title} disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal" /></label><label className="text-sm font-bold text-slate-700">Resumen del cambio<input value={draft.change_summary ?? ""} disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} onChange={(event) => setDraft((current) => ({ ...current, change_summary: event.target.value || null }))} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal" /></label></div>
                      <ConsentVisualEditor content={draft.content} variables={variables} editable={canEdit && selected.template_origin !== "DENTIA_LIBRARY"} onChange={(content) => setDraft((current) => ({ ...current, content }))} />
                      <div className="grid gap-4 lg:grid-cols-3"><label className="text-sm font-bold text-slate-700">Ámbito<select value={draft.scope_type} disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} onChange={(event) => setDraft((current) => ({ ...current, scope_type: event.target.value as "GENERAL" | "SPECIFIC", site_ids: [], procedure_ids: [], specialties: [] }))} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal"><option value="GENERAL">General</option><option value="SPECIFIC">Específico</option></select></label><label className="text-sm font-bold text-slate-700">Prioridad futura<input type="number" min="0" max="1000" value={draft.priority} disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} onChange={(event) => setDraft((current) => ({ ...current, priority: Number(event.target.value) }))} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal" /></label></div>
                      {draft.scope_type === "SPECIFIC" && <div className="grid gap-4 lg:grid-cols-3"><div><p className="mb-1.5 text-sm font-bold text-slate-700">Sedes</p><CheckList disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} items={sites.map((item) => ({ id: item.id, name: item.name }))} selected={draft.site_ids} onChange={(site_ids) => setDraft((current) => ({ ...current, site_ids }))} /></div><div><p className="mb-1.5 text-sm font-bold text-slate-700">Procedimientos</p><CheckList disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} items={procedures.map((item) => ({ id: item.id, name: item.name }))} selected={draft.procedure_ids} onChange={(procedure_ids) => setDraft((current) => ({ ...current, procedure_ids }))} /></div><label className="text-sm font-bold text-slate-700">Especialidades (código:nombre)<textarea disabled={!canEdit || selected.template_origin === "DENTIA_LIBRARY"} value={draft.specialties.map((item) => `${item.code}:${item.name}`).join("\n")} onChange={(event) => setDraft((current) => ({ ...current, specialties: event.target.value.split("\n").map((line) => line.trim()).filter(Boolean).map((line) => { const [code, ...name] = line.split(":"); return { code: code.toUpperCase(), name: name.join(":").trim() || code }; }) }))} rows={6} className="mt-1.5 w-full rounded-xl border border-slate-300 p-3 font-normal" placeholder="ENDODONCIA:Endodoncia" /></label></div>}
                      <div className={`rounded-xl p-3 text-sm font-bold ${activeDraft.clinic_content_review_confirmed ? "bg-green-50 text-green-800" : "bg-amber-50 text-amber-800"}`}>{activeDraft.clinic_content_review_confirmed ? "Contenido revisado por la clínica" : "Pendiente de revisión por la clínica"}</div>
                      <div className="flex flex-wrap gap-2">{canEdit && selected.template_origin !== "DENTIA_LIBRARY" && <button type="button" onClick={() => void saveDraft()} disabled={saving} className="rounded-xl bg-dentia-primary px-4 py-2.5 text-sm font-black text-white">Guardar</button>}<button type="button" onClick={() => void validateDraft()} disabled={saving} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-black text-slate-700">Validar</button><button type="button" onClick={() => void showPreview()} disabled={saving} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-black text-slate-700">Vista previa</button>{canPublish && canReviewContent && <button type="button" onClick={() => void publishDraft()} disabled={saving} className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white">Revisar y publicar</button>}{canVoid && <button type="button" onClick={() => void versionAction(activeDraft, "void")} disabled={saving} className="rounded-xl border border-red-200 px-4 py-2.5 text-sm font-black text-red-700">Anular borrador</button>}</div>
                    </div>}
                  </div>
                )}
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-lg font-black text-slate-950">Historial de versiones</h3><div className="mt-4 space-y-3">{allVersions.length === 0 ? <p className="text-sm text-slate-500">Sin versiones visibles.</p> : [...allVersions].sort((a, b) => b.version_number - a.version_number).map((version) => <div key={version.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-slate-900">Versión {version.version_number} · {statusLabel[version.status]}</p><p className="mt-1 text-xs text-slate-500">{new Date(version.updated_at).toLocaleString()} · {version.change_summary || "Sin resumen de cambio"}</p><p className={`mt-2 text-xs font-bold ${version.clinic_content_review_confirmed ? "text-green-700" : "text-amber-700"}`}>{version.clinic_content_review_confirmed ? "Contenido revisado por la clínica" : "Pendiente de revisión por la clínica"}</p>{version.content_sha256 && <p className="mt-1 break-all font-mono text-[10px] text-slate-400">SHA-256 {version.content_sha256}</p>}{version.legacy_quarantined && <p className="mt-2 rounded-lg bg-red-50 p-2 text-xs font-bold text-red-700">{version.legacy_quarantine_message ?? "Versión anterior no apta para nuevos consentimientos"}</p>}{version.retire_reason && <p className="mt-2 text-xs text-red-700">Retiro: {version.retire_reason}</p>}{version.void_reason && <p className="mt-2 text-xs text-red-700">Anulación: {version.void_reason}</p>}</div><div className="flex flex-wrap gap-2">{canReviewContent && !version.clinic_content_review_confirmed && (version.status === "DRAFT" || version.status === "PUBLISHED") && <button type="button" disabled={saving} onClick={() => void reviewExistingVersion(version)} className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-1.5 text-xs font-black text-amber-800">Confirmar revisión clínica</button>}{canCreate && version.status !== "VOIDED" && <button type="button" onClick={() => void versionAction(version, "draft")} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-black">Crear borrador desde versión</button>}{canRetire && version.status === "PUBLISHED" && <button type="button" onClick={() => void versionAction(version, "retire")} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-black text-red-700">Retirar</button>}</div></div></div>)}</div></div>
              </div>
            )}
          </section>
        </div>
