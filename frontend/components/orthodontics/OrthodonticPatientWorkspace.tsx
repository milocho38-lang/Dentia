...
const statusLabel: Record<OrthodonticCase["display_status"], string> = {
  ...
  COMPLETED: "Finalizado", DISCONTINUED: "Interrumpido",
};
...
export function OrthodonticPatientWorkspace({ patientId, initial }: { patientId: string; initial: Workspace }) {
  ...
  return <div>
    ...
    {tab !== "summary" && <section className="rounded-2xl border bg-white p-8 text-center"><h2 className="font-black">{tab === "evolution" ? "Evolución de Ortodoncia" : workspace.record_label}</h2><p className="mt-2 text-sm text-slate-500">Disponible en la próxima fase.</p></section>}
    ...
    {tab === "summary" && current && <>
      ...
      <section className="mt-5 grid gap-4 md:grid-cols-2"><EmptyBlock title="Última visita" text="Sin evoluciones registradas." /><EmptyBlock title="Próxima sesión" text="Sin indicaciones clínicas registradas." /><EmptyBlock title="Alertas" text="Sin alertas ortodóncicas activas." /><div className="rounded-2xl border bg-white p-5"><h3 className="font-black">Próxima cita agendada</h3><p className="mt-2 text-sm text-slate-600">{workspace.summary?.next_appointment ? `${date(workspace.summary.next_appointment.starts_at)} · ${workspace.summary.next_appointment.reason}` : "Sin próxima cita agendada."}</p></div></section>
      {current.status !== "COMPLETED" && <section className="mt-5 rounded-2xl border bg-white p-5"><h3 className="font-black">Acciones del caso</h3><div className="mt-4 flex flex-wrap gap-2">{current.status === "DRAFT" && <button type="button" disabled={busy} onClick={() => void run(() => activateOrthodonticCase(current.id, current.row_version))} className="rounded-xl bg-green-700 px-4 py-2 font-bold text-white">Iniciar tratamiento</button>}{current.status === "ACTIVE" && <><button type="button" disabled={busy} onClick={() => void run(() => suspendOrthodonticCase(current.id, current.row_version))} className="rounded-xl border px-4 py-2 font-bold">Suspender</button><button type="button" disabled={busy} onClick={() => void run(() => completeOrthodonticCase(current.id, current.row_version))} className="rounded-xl border px-4 py-2 font-bold">Finalizar</button></>}{current.status === "SUSPENDED" && <button type="button" disabled={busy} onClick={() => void run(() => resumeOrthodonticCase(current.id, current.row_version))} className="rounded-xl bg-green-700 px-4 py-2 font-bold text-white">Reanudar</button>}</div>{current.status === "ACTIVE" && <div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Motivo obligatorio de interrupción" className="min-h-11 flex-1 rounded-xl border px-3" /><button type="button" disabled={busy || !reason.trim()} onClick={() => void run(() => discontinueOrthodonticCase(current.id, current.row_version, reason))} className="rounded-xl border border-red-200 px-4 py-2 font-bold text-red-700 disabled:opacity-40">Interrumpir caso</button></div>}
      ...
      {current.status === "COMPLETED" && <section className="mt-5 rounded-2xl border bg-slate-50 p-5"><h3 className="font-black">Caso cerrado</h3><p className="mt-2 text-sm">{date(current.completed_at)}{current.closure_notes ? ` · ${current.closure_notes}` : ""}</p></section>}
    </>}
...
function EmptyBlock({ title, text }: { title: string; text: string }) { return <div className="rounded-2xl border bg-white p-5"><h3 className="font-black">{title}</h3><p className="mt-2 text-sm text-slate-500">{text}</p></div>; }
